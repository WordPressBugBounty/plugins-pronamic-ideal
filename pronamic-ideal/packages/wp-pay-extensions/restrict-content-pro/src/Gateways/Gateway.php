...
class Gateway extends RCP_Payment_Gateway {
	...
	public function get_label() {
		...
		$label = (string) PaymentMethods::get_name( $this->payment_method, __( 'Pronamic', 'pronamic-ideal' ) );
	...
	public function payments_settings( $rcp_options ) {
		...
		<table class="form-table">
			...
			<tr>
				<th>
					<label for="rcp_settings[<?php echo esc_attr( $this->id . '_label' ); ?>]"><?php esc_html_e( 'Checkout label', 'pronamic-ideal' ); ?></label>
				</th>
				<td>
					...
					<p class="description"><?php esc_html_e( 'Enter a label to display at checkout.', 'pronamic-ideal' ); ?></p>
				</td>
			</tr>

			<tr>
				<th>
					<label for="rcp_settings[<?php echo esc_attr( $config_option ); ?>]"><?php esc_html_e( 'Gateway Configuration', 'pronamic-ideal' ); ?></label>
				</th>
				<td>
					<?php
					...
					<p class="description"><?php esc_html_e( 'Choose your configuration.', 'pronamic-ideal' ); ?></p>
				</td>
			</tr>
		</table>
	...
	public function process_signup() {
		...
		if ( empty( $gateway ) ) {
			...
			\wp_die(
				...
				\esc_html__( 'Payment Error', 'pronamic-ideal' ),
				[ 'response' => 401 ]
			);
		}
			...
			\wp_die(
				\esc_html(
					\sprintf(
						...
						__( 'Payment creation failed before sending buyer to the payment provider. Error: %s', 'pronamic-ideal' ),
						$e->getMessage()
					)
				),
				\esc_html__( 'Payment Error', 'pronamic-ideal' ),
