	...
	public function handle_http_post_push( \WP_REST_Request $request ) {
		...
		if ( ! \array_key_exists( 'brq_transactions', $parameters ) ) {
			return new \WP_Error(
				...
				\__( 'The BRQ_TRANSACTIONS parameter is missing from the Buckaroo push request.', 'pronamic-ideal' )
			);
		}
		...
		if ( null === $payment ) {
			return new \WP_Error(
				...
				\sprintf(
					...
					\__( 'Unable to find payment for transaction key: %s.', 'pronamic-ideal' ),
					$transaction_key
				),
				[ 'status' => 400 ]
			);
