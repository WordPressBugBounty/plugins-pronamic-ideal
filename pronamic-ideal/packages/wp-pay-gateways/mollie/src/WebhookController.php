...
class WebhookController {
	...
	private function get_webhook_status_check_note( $action_id ) {
		if ( \is_int( $action_id ) && $action_id > 0 ) {
			...
			return \sprintf(
				\__( 'Mollie webhook received. The payment status will be requested asynchronously via action ID %d.', 'pronamic-ideal' ),
				$action_id
			);
		}

		return \__( 'Mollie webhook received. The payment status will be requested immediately.', 'pronamic-ideal' );
	}
